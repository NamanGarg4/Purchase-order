...
class RequestforQuotation(BuyingController):
   ...
   #Start of Naman Code
    def update_status_on_sq_submission(self):
        # Ensures the RFQ status is always correct based on the number of submitted Supplier Quotations.
...
##### Naman Code Starts
@frappe.whitelist()
def update_to_quotation_requested(name):
    doc = frappe.get_doc("Request for Quotation", name)
    if doc.docstatus == 1:
        doc.db_set("status", "Quotation Requested")
        frappe.msgprint(_("Status updated to 'Quotation Requested'"))
###### Naman code Ends
...
def send_supplier_emails(rfq_name):
    ...
    if rfq.docstatus == 1:
        allowed_statuses = ["Submitted", "Quotation Received", "Quotation Partially Received"]
        if rfq.status in allowed_statuses:
            rfq.db_set("status", "Quotation Requested")
            frappe.msgprint(_("Status updated to 'Quotation Requested'"))
